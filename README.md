...
スクリーンショットや発表時に、Vector Store IDやFile IDをマスク表示できます。

公開用資料・発表用資料・就活ポートフォリオで画面を見せることを想定し、APIキーや内部IDが画面に映らないようにしています。
